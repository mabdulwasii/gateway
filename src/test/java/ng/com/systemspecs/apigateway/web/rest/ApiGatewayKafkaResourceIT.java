...
class ApiGatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        ApiGatewayKafkaResource kafkaResource = new ApiGatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/api-gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/api-gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
